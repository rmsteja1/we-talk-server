...
router.put("/:id/profilePicture", upload.single("profilePicture"), async (req, res) => {
  ...
  try {
    ...
    user.profilePicture = req.body.name;
    await user.save();
...
router.put("/:id/coverPicture", upload.single("coverPicture"), async (req, res) => {
  console.log('Reached cover picture')
  // if (req.body.userId === req.params.id || req.body.isAdmin) {
  try {
    const file = req.file
    console.log(file)
